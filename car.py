...
class Car:
    ...
    def drawSensors(self, world, road, width=2):
        ...
        # Draw rays + hit points
        origin = world.getScreenCoords(self.x, self.y)
        for k, s in enumerate(sensors):
            omega = radians(self.rot + 45*k)
            dx = s * sin(omega)
            dy = - s * cos(omega)

            end = world.getScreenCoords(self.x + dx, self.y + dy)
            py.draw.line(world.win, (20, 120, 200), origin, end, width)

            # If it hit something, draw a small dot at intersection
            if s < SENSOR_DISTANCE:
                py.draw.circle(world.win, (200, 60, 60), end, 4)
